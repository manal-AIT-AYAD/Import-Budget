...
def transform_budget_data_append_sheet(input_files, existing_file, new_sheet_name="Import Odoo"):
    ...
    for input_file in input_files:
        ...
        try:
            df_source = pd.read_excel(input_file, header=2)
        ...
        colonnes_presentes = df_source.columns.str.lower().str.replace(' ', '').str.replace('é', 'e').str.replace('è', 'e').str.replace('û', 'u').str.replace('ô', 'o')
        mois_map = {}
    ...
    for annee_budget in sorted(all_data_by_year.keys()):
        ...
        compteur_global = 1  # Reset global compteur par année
...
if __name__ == "__main__":
    input_files = ["compte_de_resultats_budget.xlsx"]
    output_file = "compte_de_resultats_budget.xlsx"
